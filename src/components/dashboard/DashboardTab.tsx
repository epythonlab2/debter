  ...
  return (
    <div className="space-y-5 pb-2 text-slate-700 antialiased font-sans w-full p-0 m-0">
        ...
        {(['today', 'yesterday', 'week', 'month'] as TimeFilterType[]).map((filterOpt) => {
          ...
          return (
            <button
              ...
              className={`flex-1 text-center py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 cursor-pointer ${
                isActive 
      ...
      >
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2 bg-white/10 px-3.5 py-1.5 rounded-full backdrop-blur-md border border-white/10">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-blue-50">{t.dubeDebt || "Outstanding Credit"}</span>
          </div>
          <span className="text-xs font-bold bg-white/15 text-white px-3 py-1 rounded-lg border border-white/10 backdrop-blur-md uppercase tracking-wider">
            {t.unpaidLedger}
          </span>
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm font-bold uppercase tracking-wider text-blue-200/80">{t.currency}</span>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-2">
          <div className="space-y-2">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">{t.currency}</span>
            </div>
            
            <span className="text-xs font-bold text-emerald-700 bg-emerald-50/60 px-2.5 py-1 rounded-md border border-emerald-100/50 flex items-center w-fit gap-1.5 select-none uppercase tracking-wider">
          ...
          <div className="w-full sm:w-auto text-left sm:text-right space-y-1.5 bg-slate-50/80 p-4 rounded-2xl border border-slate-100/70">
            <span className="text-xs text-slate-400 font-bold uppercase block tracking-widest">
              {timeFilter === 'week' ? "Weekly Goal" : timeFilter === 'month' ? "Monthly Goal" : t.dailyGoalLabel}
            </span>
        ...
        <div className="flex items-center gap-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-100/80">
          ...
          <div className="flex-1 space-y-2">
            <div className="flex justify-between text-xs tracking-tight">
              <span className="flex items-center gap-1.5 font-bold uppercase text-[#1a5fb4]">
        ...
        <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-100 text-xs text-slate-400 font-bold uppercase tracking-widest">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs space-y-1.5">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-bold uppercase tracking-wider block">{t.totalRevenue || "Total"}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs space-y-1.5">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-bold uppercase tracking-wider block">{t.unitsSold || 'Units'}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs space-y-1.5">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-bold uppercase tracking-wider block">{t.avgValue || 'Ticket'}</span>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs space-y-4">
        <h4 className="font-bold text-xs text-slate-800 uppercase tracking-widest flex items-center gap-2">
                ...
                <div key={idx} className="space-y-2">
                  <div className="flex justify-between items-center text-sm font-medium text-slate-700 gap-4">
                    ...
                    <span className="font-extrabold text-slate-900 shrink-0 font-mono">
                      {p.value.toLocaleString()} <span className="text-xs text-slate-400 font-bold font-sans uppercase">{t.currency}</span>
                    </span>
                  </div>
          ...
          <div className="flex justify-between items-end h-32 pt-4 gap-3">
            {last7Days.map((d, idx) => {
              ...
              if (idx === last7Days.length - 1) { 
                translatedDateLabel = t.todayLabel || t.am?.todayLabel || t.en?.todayLabel || "Today";
              } else {
                const dayKey = d.date as 'Mon' | 'Tue' | 'Wed' | 'Thu' | 'Fri' | 'Sat' | 'Sun';
                translatedDateLabel = t.days?.[dayKey] || t.am?.days?.[dayKey] || t.en?.days?.[dayKey] || d.date;
              }
